...
export default function TutorialPage() {
	const [didStartConfiguration, setDidStartConfiguration] = useState<boolean>(false);
...
function WelcomeStep({ setStart }: { setStart: (b: boolean) => void }) {
	return (
		<div className="h-screen flex flex-col justify-center items-center">
			...
			<Button onClick={() => setStart(true)}>Create your first workspace</Button>
		</div>
	);
}
	...
	return (
		<div className="flex flex-col justify-center">
			<div className="mb-4">
				<Button
					variant={"ghost"}
					className={cn('px-0 mb-6 text-sm text-gray-400 hover:text-gray-800 transition-colors', {
			...
			</div>
			{
				currentStep === SECTIONS_INPUTS.length - 1 ?
					(<Link href={'/dashboard'} className="ml-auto"> <Button className="w-fit">Finish</Button></Link>) :
		...
		<div>
			...
			<div className="grid xl:grid-cols-3 items-center gap-8 shadow-sm p-8 bg-white rounded-md">
				<div
					...
					style={{
						backgroundImage: `url(${workspace.image_url || "https://placehold.co/150x150"
							})`,
